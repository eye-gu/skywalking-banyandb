...
The SkyWalking OAP is using gRPC to communicate with the liaison node. Ensure that the gRPC port is open and accessible from the client application. If you are using the [Canopy](../../interacting/canopy.md) web console or bydbctl to connect to the liaison node, ensure that the correct HTTP port is used for the connection. The default HTTP port is `17913`. Refer to the [network](../configuration.md#liaison--network) for more details.
